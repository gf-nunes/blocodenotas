...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=blocodenotas.example.com
...
workon blocodenotas  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development blocodenotas npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop blocodenotas
    docker rm blocodenotas
    docker run --name blocodenotas -d --env-file /home/ubuntu/blocodenotas.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/blocodenotas:/dkdata \
        blocodenotas start_web.sh
}
...
echo_green "Welcome to blocodenotas's dev env"
